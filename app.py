...
import streamlit as st
from tutor_agent import setup_mental_health_agent

st.set_page_config(page_title="Mental Health Support Chatbot", page_icon="💬")

st.title("💬 Mental Health Support Chatbot")
st.markdown("""
This AI assistant provides general mental health information and support. It is **not a substitute for professional help**. If you are in crisis, please contact emergency services or a crisis hotline immediately.
""")

# Initialize the agent only once
@st.cache_resource(show_spinner=True)
def get_agent():
    return setup_mental_health_agent()

agent = get_agent()

if agent is None:
    st.error("The chatbot agent could not be initialized. Please check your model and environment setup.")
    st.stop()
...
if submit and user_input.strip():
    with st.spinner("Thinking..."):
        try:
            result = agent({"question": user_input, "chat_history": st.session_state['chat_history']})
