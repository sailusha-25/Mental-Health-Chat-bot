...
try:
    ...
    from transformers.pipelines import pipeline  # FIXED: import pipeline from correct module
...
def load_mental_health_dataset(max_size: Optional[int] = None) -> Any:  # FIXED: Optional[int]
    ...
    try:
        ...
        # Only call len() if dataset is sized
        try:
            from datasets import Dataset, DatasetDict
            if isinstance(dataset, (Dataset, DatasetDict)):
                dataset_len = len(dataset)
                logger.info(f"✅ Loaded {dataset_len} mental health examples")
            else:
                logger.info(f"✅ Loaded mental health dataset (unknown size)")
        except Exception:
            logger.info(f"✅ Loaded mental health dataset (unknown size)")
        return dataset
    except Exception as e:
...
if __name__ == "__main__":
    try:
        ...
        if agent is None:
            logger.error("❌ Agent setup failed. Exiting test.")
            print("Error: Agent setup failed.")
        else:
